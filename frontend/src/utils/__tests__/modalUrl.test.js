...
test('parseModalParams reads tour and lightbox state', () => {
  ...
  assert.deepEqual(parseModalParams('?modal=PHOTO_TOUR_SCROLLABLE&modalItem=1042'), {
    tour: true,
    index: 42,
  });
});
...
test('parseModalParams ignores items without the tour or out of range', () => {
  ...
  assert.deepEqual(parseModalParams('?modal=PHOTO_TOUR_SCROLLABLE&modalItem=999'), {
    tour: true,
    index: null,
  });
  assert.deepEqual(parseModalParams('?modal=PHOTO_TOUR_SCROLLABLE&modalItem=abc'), {
    tour: true,
    index: null,
  });
...
test('buildModalHref writes and clears params while keeping others', () => {
  ...
  assert.equal(
    buildModalHref(base, { tour: true, index: null }),
    '/rooms/1?ref=email&modal=PHOTO_TOUR_SCROLLABLE#reviews',
  );
  assert.equal(
  ...
  assert.equal(
    buildModalHref('http://localhost/?modal=PHOTO_TOUR_SCROLLABLE&modalItem=1007', { tour: false }),
    '/',
  );
});
